...
export default function ReportPage() {
  ...
  const checkAndRegisterFileHash = (file: File): boolean => {
    try {
      ...
      const storedHashes = JSON.parse(
        localStorage.getItem("infrashield_uploaded_hashes") || "[]"
      );
      if (storedHashes.includes(hash)) return false;
      storedHashes.push(hash);
      localStorage.setItem(
        "infrashield_uploaded_hashes",
        JSON.stringify(storedHashes.slice(-50))
      );
      return true;
    } catch {
      return true;
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    if (!isUnique) {
      ...
      setErrorMessage(
        "DUPLICATE DETECTED: You have already uploaded this exact photograph. Request blocked and 0 Points awarded."
      );
      return;
    }
  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setErrorMessage(
          "Unable to retrieve GPS. Please enter your location address manually."
        );
        setIsLocating(false);
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!imagePreview) {
      setErrorMessage("Please upload a photo of the issue.");
      return;
    }
    if (!address.trim() && !coordinates) {
      setErrorMessage(
        "Please click 'Auto-Detect GPS' or type a location address before submitting."
      );
      return;
    }
    ...
    try {
      ...
      if (
        !analyzeRes.ok ||
        !analyzeData.success ||
        analyzeData.data?.pointsAwarded === 0
      ) {
        setIsPersonalOrSpam(true);
        setErrorMessage(
          analyzeData.message ||
            "REJECTED: Please upload a clear photo of real infrastructure damage. 0 Points Awarded."
        );
      ...
      setIsPersonalOrSpam(true);
      setErrorMessage(
        "REJECTED: Unable to analyze photo. Please ensure it shows clear infrastructure damage. 0 Points Awarded."
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  if (submittedTicket && analysisResult) {
    ...
    const departmentName =
      analysisResult?.recommendation?.department || "Public Works Department";
      ...
      <div className="min-h-[85vh] bg-[#f8fafc] flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-xl bg-white border border-slate-200 rounded-3xl p-8 shadow-xl text-center space-y-6">
          ...
          <div className="space-y-2">
            <span className="text-xs font-bold text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full uppercase tracking-wider border border-emerald-100 inline-flex items-center gap-1">
              <Sparkles className="h-3.5 w-3.5" /> Report Submitted (+50 PTS)
            </span>
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">
              Thank You for Reporting!
            </h1>
            <p className="text-slate-500 text-sm font-medium">
              Your report has been dispatched to the responsible municipal department.
            </p>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 text-left space-y-3 text-xs font-semibold text-slate-600">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-slate-950 text-right w-1/2 truncate">
                {departmentName}
              </span>
            </div>
          </div>
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="border-b border-slate-200 pb-6 text-center sm:text-left">
          ...
          <p className="text-slate-500 text-sm mt-1">
            Upload a photo of damaged roads, broken bridges, or drainage problems to notify city officials.
          </p>
        </div>

        {errorMessage && (
          <div
            className={`p-5 border rounded-2xl text-xs font-semibold flex items-start gap-4 shadow-sm ${
              isPersonalOrSpam
          ...
          >
            ...
            <div className="flex-1 space-y-1">
              <span className="block font-bold text-sm text-rose-900">
                {isPersonalOrSpam
                  ? "Upload Rejected: Invalid Photograph"
                  : "Validation Error"}
              </span>
              ...
              {isPersonalOrSpam && (
                <button
                  ...
                  className="mt-2 px-3 py-2 bg-rose-600 hover:bg-rose-500 text-white rounded-xl text-xs font-bold transition-colors"
                >
            ...
            {!imagePreview ? (
              ...
              >
                ...
                <span className="text-xs text-slate-400 block mt-1">
                  Upload potholes, road cracks, or drainage issues. Personal photos and non-infrastructure images are rejected.
                </span>
              </div>
            ) : (
              <div className="relative rounded-2xl overflow-hidden border border-slate-200 bg-slate-900 h-72 w-full">
                ...
                <div className="absolute top-4 right-4 flex gap-2">
                  <button
                    type="button"
                    onClick={handleRemoveImage}
                    className="bg-rose-600 text-white p-1.5 rounded-xl shadow-md hover:bg-rose-500 transition-colors"
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                {Object.entries(DEFECT_CATEGORIES).map(([key, item]: [string, any]) => (
                  <button
                    ...
                    className={`flex items-center gap-2.5 p-3.5 rounded-2xl border text-xs font-bold transition-all ${
                      category === key
                  ...
                  >
                    ...
                    <span className="truncate">
                      {item.label ? item.label.split("/")[0] : item.name}
                    </span>
                  </button>
                ))}
              </div>
