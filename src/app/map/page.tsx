            ...
            {idx < steps.length - 1 && (
              <div
                className={`h-0.5 flex-1 mx-1 rounded ${
                  stepNum < info.step ? "bg-emerald-400" : "bg-slate-200"
                }`}
              />
            )}
          </div>
...
export default function MapPage() {
  ...
  const inProgressCount = issues.filter(
    (i) => (i.status || "").toUpperCase() === "IN_PROGRESS"
  ).length;
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold text-blue-600 uppercase tracking-wider">
              ...
              {role === "official"
                ? "Live City Map • Real-Time Updates"
                : "My Reported Issues • Live Tracker"}
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            <button
              onClick={fetchIssues}
              className="p-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 hover:text-blue-600 rounded-lg transition-colors flex items-center gap-1"
              title="Refresh"
            >
              <RefreshCw
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search by street or issue..."
                className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-4 py-2.5 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white transition-all"
              />
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-1.5">
                <button
                  ...
                  className={`py-1.5 px-3 text-xs font-bold rounded-lg text-left transition-all ${
                    categoryFilter === "ALL"
                ...
                {Object.entries(DEFECT_CATEGORIES).map(([key, item]: [string, any]) => (
                  <button
                    key={key}
                    onClick={() => setCategoryFilter(key)}
                    className={`py-1.5 px-3 text-xs font-bold rounded-lg text-left border truncate transition-all ${
                      categoryFilter === key
                        ? "bg-gradient-to-r from-blue-50 to-purple-50 border-blue-200 text-blue-700"
                        : "bg-slate-50 hover:bg-slate-100 border-slate-200 text-slate-500"
                    }`}
                  >
                    {item.icon} {item.label ? item.label.split("/")[0] : item.name}
                  </button>
                ))}
              </div>
            </div>
                    ...
                    <div
                      ...
                      className={`p-3 rounded-xl border transition-all cursor-pointer ${
                        selectedIssue?.id === issue.id
                          ...
                          : "bg-slate-50/50 border-slate-200/60 hover:border-blue-200 hover:bg-white hover:shadow-sm"
                      }`}
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        <span
                          className={`text-[9px] font-bold px-2 py-0.5 rounded tracking-wider uppercase border ${statusInfo.color}`}
                        >
                      ...
                      <div className="flex items-center gap-1 text-[11px] text-slate-500 truncate font-medium">
                        ...
                        <span className="truncate">
                          {issue.location?.address || "Location Saved"}
                        </span>
                      </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl max-w-2xl w-full p-6 space-y-5 shadow-2xl text-slate-800 relative max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span
                  className={`text-xs font-bold px-2.5 py-1 rounded border ${
                    getStatusInfo(selectedIssue.status).color
                  }`}
                >
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 text-center">
              ...
              >
                ...
                <div className="pt-1 text-xs space-y-1.5 font-medium">
                  <div className="text-slate-600">
                    ...
                    {selectedIssue.vision?.probableCause || "Under review"}
                  </div>
                  <div className="text-slate-600">
                    <strong className="text-slate-500">Department:</strong>{" "}
                    {selectedIssue.recommendation?.department ||
                      "Public Works Department"}
                  </div>
                  <div className="text-emerald-700 font-bold">
                    <strong className="text-slate-500 font-normal">
                      Est. Cost:
                    </strong>{" "}
                    {selectedIssue.recommendation?.estimatedBudgetRange?.replace(
                      /\$/g,
                      "₹"
                    ) || "₹45,000"}
                  </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-lg font-black text-blue-800">
                    {getETA(selectedIssue.priority?.score || 50)}
                  </span>
                </div>
              </div>
              <div className="text-right">
                ...
                <span className="text-lg font-black text-rose-600">
                  {selectedIssue.priority?.score || 50}/100
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-2 border-t border-slate-100">
              <button
                onClick={() => setSelectedIssue(null)}
                className="flex-1 py-2.5 px-4 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-bold rounded-xl transition-colors"
              >
                Close
              </button>
              <button
                onClick={() => {
                  generateEngineeringReport({
                    reportId: `INFRA-${selectedIssue.id
                      .substring(0, 6)
                      .toUpperCase()}`,
                    date: new Date(
                      selectedIssue.createdAt || Date.now()
                    ).toLocaleDateString("en-IN"),
                    location:
                      selectedIssue.location?.address || "Location Saved",
                    gpsCoordinates: selectedIssue.location
                      ? `${selectedIssue.location.lat.toFixed(
                          4
                        )}° N, ${selectedIssue.location.lng.toFixed(4)}° E`
                      : "16.3067° N, 80.4365° E",
                    defectType:
                      selectedIssue.vision?.issueType || "Reported Issue",
                    severity: selectedIssue.vision?.severity || "medium",
                    severityScore: selectedIssue.priority?.score || 50,
                    description:
                      selectedIssue.executiveSummary?.summary ||
                      "Inspection needed.",
                    aiAnalysis:
                      selectedIssue.vision?.probableCause ||
                      "Pending evaluation.",
                    estimatedCost: 45000,
                    recommendedAction:
                      selectedIssue.recommendation?.temporarySolution ||
                      "Deploy barricades.",
                    department:
                      selectedIssue.recommendation?.department ||
                      "Public Works Department",
